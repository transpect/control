...
declare
  ...
  %rest:form-param("svnurl", "{$svnurl}")
function control-forms:upload($file, $svnurl) {
...
declare
  ...
  %rest:query-param("svnurl", "{$svnurl}")
function control-forms:download-as-zip( $svnurl as xs:string ) {
...
declare
  ...
  %output:method('html')
function control-forms:copy( $svnurl as xs:string, $file as xs:string ) {
<html>
  <head>
    {control-widgets:get-html-head( )}
  </head>
